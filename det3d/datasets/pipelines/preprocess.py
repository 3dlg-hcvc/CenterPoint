...
@PIPELINES.register_module
class Track3DVoxelization(object):
    def __init__(self, **kwargs):
        cfg = kwargs.get("cfg", None)
        self.range = cfg.range
        self.voxel_size = cfg.voxel_size
...
@PIPELINES.register_module
class Track3DAssignLabel(object):
    def __init__(self, **kwargs):
        """Return CenterNet training labels like heatmap, height, offset"""
        assigner_cfg = kwargs["cfg"]
        self.out_size_factor = assigner_cfg.out_size_factor
    ...
    def __call__(self, res, info):
        ...
        if res["mode"] == "train":
            # Calculate output featuremap size
            # Calculate output featuremap size
            grid_size = res["voxels"]["shape"]
            pc_range = res["voxels"]["range"]
            voxel_size = res["voxels"]["size"]
            feature_map_size = grid_size[:2] // self.out_size_factor

            # We only have one class add one box for each frame
            task_boxes = [[res["annotations"]]]
            task_classes = [[1]]
            task_names = [["objects"]]
            ...
            for idx, task in enumerate(self.tasks):
                ...
                # In our case, the number of obb is always 1
                assert len(task_boxes[idx]) == 1
                num_objs = 1
